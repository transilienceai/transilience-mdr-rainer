...
python .claude/skills/normalize-observations/scripts/normalize_cloudtrail.py \
  --input examples/cloudtrail.sample.json \
  --output outputs/normalized_observations.jsonl

python .claude/skills/raw-evidence-pack/scripts/package_raw_evidence.py \
  --input examples/cloudtrail.sample.json \
  --output outputs/evidence_pack \
  --case-config examples/cases.example.json

python .claude/skills/business-baseline/scripts/build_business_baseline.py \
  --observations outputs/normalized_observations.jsonl \
  --output outputs/baseline.json \
  --customer "Example"

python .claude/skills/detection-specs/scripts/generate_detection_specs.py \
  --baseline outputs/baseline.json \
  --output outputs/cloudtrail_detection_specs.json

python .claude/skills/business-triage/scripts/triage_cloudtrail_findings.py \
...
python .claude/skills/report-packager/scripts/package_markdown_report.py \
  --customer "Example" \
  --baseline outputs/baseline.json \
  --evidence-pack outputs/evidence_pack \
